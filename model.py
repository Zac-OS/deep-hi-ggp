...
class Model:
    ...
    def create_model(self):
        ...
        self.input = tf.compat.v1.placeholder(
            shape=[None, self.num_inputs], dtype=tf.float32)
        cur = self.input
    ...
    def create_trainer(self):
        ...
        for role, (q, logits, probs) in self.outputs.items():
            ...
            self.loss += tf.reduce_mean(input_tensor=tf.compat.v1.losses.softmax_cross_entropy(
                onehot_labels=tprobs, logits=logits))
            self.loss += tf.reduce_mean(
                input_tensor=tf.compat.v1.losses.mean_squared_error(q, tq))

        # Add weight regularisation?
        vars = tf.compat.v1.trainable_variables()
        self.C = 0.0001
        lossL2 = tf.add_n([tf.nn.l2_loss(v)
                          for v in vars if 'bias' not in v.name]) * self.C
        self.loss += lossL2
